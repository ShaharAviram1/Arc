...
- Per-process caps (`jobs/loop.py`, `process_caps`): the loop counts what it
  ...
  claim already made). One INFO line when the gate closes and one when it
  opens, not one per poll.
...
M16's failure banner (FR-W6, §5.4b) adds **no table and no column**: it is a
read of `wants` + `episodes` and one of `mal_write_log`, and the one thing it
stores — which rows a reader has dismissed — lives in that browser's
...
- `search_release`: `wanted→searching`; Nyaa search + filter + rank; top
  ...
  `downloading`. One attempt is up to 10 query forms and, for a `FINISHED`
  show that found fewer than three candidates, up to 9–18 group-narrowed forms
  after them (§6) under a 20-request ceiling — some 40 s of paced requests,
  which nothing in the queue reads as stuck: there is no
  per-handler timeout, `WORKER_STALE_AFTER` (2 h) bounds *silence* rather than
  work, and the worker's 30 s heartbeat is its scheduler's own tick on the same
  event loop. No candidate → requeue itself (same dedupe key): every 30
...
- Two encodes per worker process (`MAX_TRANSCODES`), and the claim loop
  enforces it *before* the claim: while this process already runs that many
  transcodes it asks for anything but a transcode, so a queued encode stays
  `pending` instead of taking a concurrency slot and parking on the semaphore
  (§2 "Per-process caps"; the incident is in the decision log for
  2026-09-18). The semaphore is unchanged and still the guard for a second
  worker process on the host and for the cap being lowered under a claim
  already made. The handler heartbeats
...
schema change**: both halves are reads of tables §4 already has —
`wants` + `episodes` for the first, `mal_write_log` for the second — and
nothing is written, not even a dismissal.

- **The rule is one pure query function**,
  `arc/services/catalog/failures.py::failures_for_user`, next to
  `progress.py` because it answers the same kind of question about the same
  page. Two queries, and a third only when it is needed:
  1. `FROM anime JOIN episodes JOIN wants` where the want is the caller's and
     **live** (`dropped_at IS NULL`, samples included), and the episode's state
     is `failed` or `unavailable` — the two states in spec §6 that mean
     "stopped, and not because it is finished". `wants` is keyed
     `(user_id, episode_id)`, so one row per episode falls out of the schema
     and there is nothing to deduplicate. Ordered `state_changed_at DESC NULLS
     LAST, id DESC`, capped at 20.
  2. `FROM mal_write_log JOIN anime` where the row is the caller's and
     `status = failed`, newest first, capped at 10 (`MAL_LIMIT`) — a push that
     failed on a whole list must not be the entire banner. `pending` is a write
     still on its way and `skipped` is FR-M5's non-write; neither is a failure.
     Nothing is excluded for having been *reverted*, because nothing can have
     been: a revert is only offered on a write that succeeded (`writelog.
     is_revertible` refuses anything but `ok`).
  3. `latest_transcode_jobs` — the same `DISTINCT ON (payload->>'episode_id')`
     the show page uses — for the episodes that came back `failed`, so the
     reason is the transcode's own `error_tail` (then `last_error`), trimmed
     with `trim_middle` to `REASON_LIMIT` = 160. Skipped entirely when nothing
     is `failed`, which is why `test_home_api`'s "jobs is read twice per page"
     still holds.
- **What a row carries**: a stable `key`, the kind, an ordinary
  `AnimeSummary` (so the card is named and linked like every other card on the
  page — `null` list status is normal here, since a sample has no list entry),
  one trimmed sentence, `since`, and then either the episode's id/number/state
  or the log row's id/field/old/new. Deliberately **not** an `EpisodeOut`: the
  row names an episode rather than rendering one, so it asks nothing of
  `episode_extras`' four lookups. The failure shows join the router's existing
  `list_status_for` call rather than adding one.
- **`since`**, in order: the episode's `state_changed_at` (the moment the thing
  being reported happened), then the failing job's `finished_at`, then null —
  never invented, because a failure with a made-up date sorts above real ones.
- **The key is the dismissal contract**: `episode:<id>:<state>:<since iso>` and
  `mal:<log id>`. The state is in it because a give-up and a broken transcode
  are two pieces of news about one row; `since` is in it because FR-A6 retries
  daily and the same episode failing again is a new failure. The log's id is
  enough on its own — it is append-only, so one id is one attempt for ever.
- **Client**: `Home.tsx`'s `FailureBanner`, above the hero and below the demo
  strip, three rows and a "+N more" fold. Dismissal is `localStorage` only —
  one entry per dismissed failure under
  `arc:failure-dismissed:<user id>:<failure key>`, the demo strip's precedent
  with the failure key appended — and every access is wrapped, since the
  accessor throws in a browser set to block site data (the failure mode is
  "every row is shown", which is the right way round). On mount the prefix is
  scanned and any key the server no longer reports is **deleted**, which is
...
| `GET /api/settings`, `PUT /api/settings` | admin | the rules editor (FR-D2, FR-T5): `{values, defaults, overrides[{anime_id, title, preferred_groups, resolution}]}` for every `DEFAULT_SETTINGS` key. PUT takes a partial object of those keys and writes only what it names; 422 `{detail: [{loc: ["body", key], msg, type}]}` for an unknown key or a refused value (resolutions ∈ 2160p/1080p/720p/480p and fallback ≠ preferred — enforced only when the patch names one of the two, so a hand-edited collision does not block unrelated edits — N 0..10, K 0..50, G and D 0..365, ≤ 20 groups of ≤ 64 chars de-duplicated case-insensitively, languages 2–8 lowercase letters/dashes). Writing `acquisition_paused` goes through the same `set_paused` the pause button uses, and clearing it enqueues `compute_wants` in the same transaction, so unpausing from the editor and from the button do the same thing. Overrides are **editable** since M16 (the two routes below); listing them here is what makes the global rules page honest about the shows that do not follow it. `min_free_gb` is FR-T6's storage floor: whole GB, 0..1000, 0 turning the guard off; validated and clamped at the same ceiling the reader uses, so what the panel accepts and what acquisition acts on cannot differ. `slot_cap_k` is FR-A10's K: 0..50, 0 meaning no cap, clamped at the same ceiling for the same reason |
| `PUT /api/settings/overrides/{anime_id}`, `DELETE /api/settings/overrides/{anime_id}` | admin | the per-show override editor (FR-A3, FR-D2, M16). PUT body `{preferred_groups: string[] | null, resolution: string | null}` (declared, `extra="forbid"`), answering the `OverrideOut` written — `{anime_id, title, preferred_groups, resolution}` — or **`null` when it named neither field**, which is a *deletion*: an empty stored row would read as a show not following the global rules while following them exactly. Same validators as the global keys (`settings.validate_override`, pure: ≤ 20 groups of ≤ 64 chars de-duplicated case-insensitively, resolution ∈ 2160p/1080p/720p/480p), same 422 shape `{detail: [{loc: ["body", field], msg, type}]}`, 404 `anime not found` for an id no `anime` row answers to. DELETE is 204 whether or not there was a row and does *not* require the show to exist — an override outliving its show is exactly the one an admin needs to remove. One INFO line per change with the previous value, like every other `settings` write. **Nothing is enqueued**: the ranker reads the row when it searches, so the next search uses the new rule and nothing already downloading is disturbed — the same promise the rules editor makes about the global keys (only *clearing the pause* queues a `compute_wants`) |
...
- **Query corpus** (`tests/fixtures/query_corpus.txt`, 2026-09-14): the other
  ...
  *when* they are asked — that is a property of the search and the running
  ...
  explicit tests in `test_nyaa.py`: the form × group list and its order, the
  preferred groups coming first and de-duplicated, the production case found
  off `Kimetsu no Yaiba - 10 HorribleSubs`, the three-candidate stop, an airing
  show that asks none, and the request ceiling — plus two in
  `test_acquisition_jobs.py` for the job end to end. It runs **offline** — `queries`, `acceptable` and `rank` are
...
- 2026-09-10 — M14: settings API (validated), job retry/cancel + summary,
  qBittorrent status, disk usage; overrides editor deferred to M16 (it landed
  there on 2026-09-18 — see the log entry of that date).
...
- 2026-09-18 — **Watch Now's own failures** (FR-W6, §5.4b, M16). The banner is
  a fifth field on `GET /api/home` rather than a route of its own, because the
  live-update path (§5.9) already invalidates that query on every episode state
  change: a second endpoint would have needed its own invalidation to say the
  same thing at the same moment, and the two queries behind it are small. The
  rule lives in `arc/services/catalog/failures.py` beside `progress.py` —
  another pure question about the same page — and the whole of it is reads: a
  live want on an episode that is `failed`/`unavailable`, the caller's failed
  `mal_write_log` rows, and the transcode job of whatever came back `failed`,
  the third only where there is something to explain (so the home page's
  "jobs is read twice" test still holds). No migration, and **dismissal never
  reaches the server**: one entry per dismissed failure in that browser's
  `localStorage`, pruned on mount against what the server still reports, which
  is bounded precisely because the key carries the moment the failure happened
  and the pruning throws away the keys of failures that are over. The
  alternative — a `dismissed_at` column — would have been a second source of
  truth about a row the episode already describes, and would have made one
  reader's attention a property of the failure itself.
- 2026-09-18 — **Per-show override editor** (FR-A3, FR-D2, M16, owner
  2026-09-18). Two routes and one field, no schema change and no migration:
  `PUT`/`DELETE /api/settings/overrides/{anime_id}` over
  `settings.write_override`/`delete_override`, and an admin-only `override` on
  `GET /api/anime/{id}`. The read went **on the show payload** rather than into
  a `GET` of its own so the show page's query count is unchanged — the editor
  is on a page that was already fetching the show, and a second endpoint would
  have been a second round trip and a second thing to keep in step. The
  writers reuse `validate`'s own two field validators, so what an override may
  say and what a global key may say cannot drift; `OverrideOut` moved to
  `arc/api/schemas.py` because two routers now answer with it. An override
  naming neither field is a deletion (the empty row would be a lie in the admin
  table), `DELETE` is idempotent and does not require the show to exist, and
  neither route enqueues anything — rule changes have always applied to the
  next search. Client: `useSaveOverride`/`useRemoveOverride` plus
  `parseGroups`/`overrideSummary` in `lib/admin.ts` (imported by the show page,
  the first non-admin page to use that module, because the editor is on both
  screens), the chip and inline form in `pages/Show.tsx`, and Edit/Remove per
  row in `components/admin/RulesTab.tsx`.
- 2026-09-18 — **Nyaa is searched by group for a finished show** (FR-A4, §6,
  M16, owner 2026-09-18: *"the helper will compare sources to find the most
  optimal"*). Arc saw only the newest 75 matches of each query form. That is
  the whole answer for an airing show and the wrong answer for a finished one:
  *Kimetsu no Yaiba* episode 10 came back as seven forms, 91 merged results and
  **no season-one single at all**, because everything the franchise has
  uploaded since 2019 sits in front of them. The first attempt asked for pages
  2 and 3 — and **the RSS endpoint ignores `p=`** (and `s=`/`o=`): the three
  pages are byte-identical, so that was 17 requests for the same 91 results,
  and only the HTML listing paginates. Since Nyaa ANDs every word of a query
  the reachable answer is a *narrower* question, so `nyaa.group_queries` asks
  the top three title forms again with a group's name on the end —
  `preferred_groups` then `DEEP_SEARCH_GROUPS`, form-major, de-duplicated
  case-insensitively — for a `FINISHED` entry only (`deep_search`), only while
  the pool holds fewer than `ENOUGH_CANDIDATES` (3), and under `MAX_REQUESTS`
  (20) counted across both kinds of form. Four things were deliberately *not*
  done: the filter was not touched (all 91 rejections were correct, and
  loosening it is how the wrong season lands), the airing path was not touched
  (its release is inside the newest 75), the narrowing was not given its own
  cap inside `MAX_QUERIES` (that cap is about spellings of the title), and the
  narrowed forms are not counted in `Search.forms` (the episode row's number
  has meant "ways of writing the title" since M6). The three built-in groups
  are the reviewable part: they are a claim about who uploads anime — it was
  HorribleSubs until 2020, SubsPlease after it, Erai-raws throughout — and they
  sit behind whatever an admin has preferred. The cost is the slot: one
  `search_release` can now hold a worker slot for some 40 s, which the queue
  tolerates because nothing puts a timeout on a handler and
  `WORKER_STALE_AFTER` bounds silence rather than work. `Search.requests` is
  the diagnostic, logged beside `forms` and `results`; it is **not** a column,
  because the two that are on the episode row are the two the show page renders.
- 2026-09-18 — **A capped job type is not claimed while its cap is full**
  (§2 "Per-process caps", §5.3a). Production runs `WORKER_CONCURRENCY=2` with
  an effective `MAX_TRANSCODES=1`; after a deploy the two lowest priorities in
  the queue were both transcodes, both were claimed, one encoded and the other
  sat in the second worker slot for 25 minutes at 0 %, parked on the media
  semaphore and heartbeating exactly as designed — so `compute_wants`,
  `mal_push`, `poll_qbit` and `search_release` all waited out an encode, and
  an episode the owner had just finished was not wanted for half an hour. The
  rule is therefore: `run_worker_loop` counts the jobs of each capped type it
  is running and excludes those types from the claim while the count is at the
  cap (`claim_statement(exclude_types=…)`, one `NOT IN`), which gives the free
  slot to the best *other* job and leaves the transcode pending in its own
